...
//    validation
        ...
        if ($("#step1-form").valid()) {
            ...
            $.ajax({
                ...
                data: {
                    token : "",
                    w: "1",
                    s1fname : $("#s1fname")[0].value,
                    s1name : $("#s1name")[0].value,
                    s1bday : $("#s1bday")[0].value,
                    s1cid : $("#s1cid")[0].value,
                    s1nid : $("#s1nid")[0].value,
                    s1gen : $(".s1gen")[0].value
                },
        ...
        if ($("#step2-form").valid()) {
            ...
            $.ajax({
                ...
                data: {
                    token : "",
                    w: "2",
                    s2f1mal : $("#s2f1mal")[0].value,
                    s2f2mal : $("#s2f2mal")[0].value
                },
                success: function(result) {
                    if (result.a === "success") {
                        var $active = $('.wizard .nav-tabs li.active');
    ...
    $('#step3-form').validate({
        ...
        messages: {
            drcode: "لطفا شماره نظام روانشناسی خود را وارد کنید"
        }
});
    $('#s3btn').on('click', function (e) {
        ...
        if ($("#step3-form").valid()) {
            $("html, body").animate({ scrollTop: 0 }, "slow");
            // var data = { 'token': "", 'w': "3",'takhasos[]' : [], 'roykard[]': [], 'drcode': $("#drcode")[0].value, 's3f2': $("#s3f2")[0].value};
            // $(".takhasos:checked").each(function() {
            //   data['takhasos[]'].push($(this).val());
            // });
            // $(".roykard:checked").each(function() {
            //   data['roykard[]'].push($(this).val());
            // });
            $.ajax({
                type: "POST",
                dataType : 'json',
    ...
    if ($("#step4-form").valid()) {
        ...
        $.ajax({
            ...
            data: {
                token : "",
                w: "4",
                fileupload : $("#fileupload")[0].value,
                s4phone : $("#s4phone")[0].value,
                s4province : $("#s4province")[0].value,
                s4city : $("#s4city")[0].value,
                s4addr : $("#s4addr")[0].value,
                s4post : $("#s4post")[0].value,
            },
            success: function(result) {
                if (result.a === "success") {
                    var $active = $('.wizard .nav-tabs li.active');
...
//    agreement
    ...
    $("#p-b").click(function() {
        ...
        var p = '<label for="p-' + id+1 + '" class="btn btn-default child">' + v + ' <input type="checkbox" checked="checked" id="p-' + id+1 + '" class="badgebox"><span class="badge">&check;</span></label>';
    ...
    $("#m-b").click(function() {
        ...
        var p = '<label for="m-' + id+1 + '" class="btn btn-default child">' + v + ' <input type="checkbox" checked="checked" id="m-' + id+1 + '" class="badgebox"><span class="badge">&check;</span></label>';
